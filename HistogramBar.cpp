...
HistogramBar::HistogramBar(
    ...
    _titleText->setPosition(
        sf::Vector2f(bounds.left, bounds.top + bounds.height / 2 - 10)
    );
    ...
    _valueText->setPosition(sf::Vector2f(
        barRect.left + barRect.width - _valueText->getGlobalBounds().width - 10,
        bounds.top + bounds.height / 2 -13
    ));
}
