...
void GuessGrid::tryInsertLetter(const char letter) {
    ...
    if (
        _currentWordIndex == _guessLetters.size() || 
        _solved ||
        _insertPosition == _guessLetters.at(_currentWordIndex).size()
    ) return;

    // Insert letter and move position one forward
    _guessLetters.at(_currentWordIndex).at(_insertPosition).setLetter(letter);
    ++_insertPosition;
}

void GuessGrid::backSpace() {
    // Do nothing if empty or solved
    ...
    for (int i = 0; i < _solution.length(); i++) {
        ...
        if (guess == _solution.at(i)) {
            ...
            if (requiredLetterCount > 0) {
                // Find the matching positions for incorrect placement 
                // and the number of correct placements
                ...
                // Flag as wrong position if within reaching the min
                if(std::find(
                    outOfPositionPositions.begin(),
                    outOfPositionPositions.end(),
                    i
                ) != outOfPositionPositions.end()) {
                    _guessLetters.at(_currentWordIndex).at(i).setSolutionState(
                        PuzzleLetter::SolutionState::WRONG_POS
                    );
                } else {
                    _guessLetters.at(_currentWordIndex).at(i).setSolutionState(
                        PuzzleLetter::SolutionState::NO_STATE
                    );
                }
            } else {
                _guessLetters.at(_currentWordIndex).at(i).setSolutionState(
...
std::vector<std::string> GuessGrid::getAllRules() const {
    ...
    for (int i = 0; i < _currentWordIndex; i++) {
        ...
        for (int j = 0; j < _solution.length(); j++) {
            if (_guessLetters
                .at(i).at(j)
                .getSolutionState() == PuzzleLetter::SolutionState::CORRECT
            ) {
                ruleStream << "*";
            }
...
std::string GuessGrid::getShareString() const {
    ...
    message << "Cody's Custom Wordle Game" << std::endl << "\"" << _solution << "\": ";
    message << (_solved ? "Solved: ": "Did not solved: ");
    message << _currentWordIndex << "/6" << std::endl << std::endl;

    // generate puzzle coloured title view 
    for (int i = 0; i < _currentWordIndex; i++) {
        std::stringstream ruleStream;
        for (int j = 0; j < _solution.length(); j++) {
            if (_guessLetters
            ...
            } else if (_guessLetters
                .at(i).at(j)
                .getSolutionState() == PuzzleLetter::SolutionState::WRONG_POS
            ) {
...
void GuessGrid::initialiseAllGuesses(
    const sf::Font& font, 
    ...
    int leftSide = _bounds.left + _bounds.width / 2 - \
        ((elementWidth + 10) * wordLength) / 2;
    ...
    for (int i = 0; i < maxGuesses; i++, posY += elementHeight + 10, posX = leftSide) {
        ...
        for (int j = 0; j < wordLength; j++, posX += elementWidth + 10) {
            row.emplace_back(PuzzleLetter(sf::IntRect(
                posX,
                posY,
                elementHeight,
                elementWidth
