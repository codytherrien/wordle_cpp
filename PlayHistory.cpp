...
void PlayHistory::loadFile() {
    ...
    if (file.fail()) {
        std::cout << "Failed to read expected data from save file (strea history). Using default values." << std::endl;
        initialiseDefault();
    }
...
void PlayHistory::saveFile() {
    ...
    if(!file.is_open()) {
        std::cout << "Failed to oen file to save you win. Will no be save." << std::endl;
        return;
    }
